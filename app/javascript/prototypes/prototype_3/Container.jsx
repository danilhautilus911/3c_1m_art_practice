...
export default class Container extends Component {
  ...
  createNewOscillator = () => {
    ...
    const oscillatorNode = audioCtx.createOscillator(); // все после знака = лежит внутри переменной oscillator

    const { oscillator } = this.state

    oscillatorNode.type = oscillator.settings.type
    oscillatorNode.frequency.setValueAtTime(oscillator.settings.frequency, audioCtx.currentTime); // value in hertz
    oscillatorNode.connect(audioCtx.destination); // подключение оссилятора в колонки, destination - колонки, кот. находятся внутри аудиоконтекста
